...
function encrypt(encryptData) {
  ...
  var enc = jsrsasign.KJUR.crypto.Cipher.encrypt(encryptData, pub, 'RSAOAEP')
...
function decrypt(decryptData) {
  ...
  var dec = jsrsasign.KJUR.crypto.Cipher.decrypt(jsrsasign.b64utohex(decryptData), prv, 'RSAOAEP')
  return dec
}

// test
console.log('rsa-oaep-sha1 加解密测试：')
